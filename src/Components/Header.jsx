...
function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[1000] backdrop-blur-xl bg-[rgba(5,5,5,0.75)] border-b border-[rgba(220,38,38,0.15)]">
      ...
      <div className="max-w-7xl mx-auto h-[80px] md:h-[90px] px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="h-14 w-14 md:h-20 md:w-20 flex items-center justify-center transition-all duration-300 group-hover:scale-105">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-[#ef4444] text-[9px] md:text-[10px] tracking-[4px] md:tracking-[5px] uppercase mt-1 md:mt-1.5 font-medium whitespace-nowrap">
               Trainer
            </p>
          </div>
        </Link>
        ...
        <nav className="hidden xl:flex items-center gap-4 2xl:gap-8">
          ...
          {navItems.map((item, index) => (
            <Link
              ...
              className="text-white hover:text-[#ef4444] transition-all duration-300 text-xs 2xl:text-sm tracking-[1.5px] uppercase relative group"
            >
              {item}
        ...
        <div className="flex items-center gap-4 relative z-50">
          ...
          {/* MOBILE MENU TOGGLE */}
          <button 
            className="xl:hidden text-white text-2xl p-2 hover:text-[#ef4444] transition-colors"
      ...
      {/* MOBILE MENU OVERLAY */}
      <div 
        className={`fixed inset-0 bg-black/95 backdrop-blur-3xl z-40 xl:hidden flex flex-col items-center justify-center gap-8 transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'opacity-100 translate-y-0 visible' : 'opacity-0 -translate-y-full invisible'}`}
      >
        <nav className="flex flex-col items-center gap-6 text-center mt-20">
          {navItems.map((item, index) => (
            <Link
              ...
              className="text-white hover:text-[#ef4444] transition-colors text-xl tracking-[3px] uppercase font-medium"
            >
              {item}
            </Link>
          ))}
          <Link
            to="/booking"
            className="mt-4 inline-flex items-center justify-center px-10 py-4 rounded-full bg-gradient-to-r from-[#7f1d1d] via-[#dc2626] to-[#ef4444] text-white font-bold uppercase tracking-[2px] shadow-[0_10px_35px_rgba(220,38,38,0.25)]"
          >
            Book Slot
          </Link>
        </nav>
